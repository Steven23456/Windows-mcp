...
interface CLIOptions {
  ...
  lang: 'typescript' | 'csharp' | 'auto';  // Language detection mode
}
...
function parseCliOptions(): CLIOptions {
  ...
  for (const arg of args) {
    if (arg.startsWith('--root=')) {
      ...
      const langVal = arg.slice(7);
      if (!['typescript', 'csharp', 'auto'].includes(langVal)) {
        console.error(`Error: --lang must be one of: auto, typescript, csharp (got: ${langVal})`);
        process.exit(1);
      }
      options.lang = langVal as 'typescript' | 'csharp' | 'auto';
...
Options:
  ...
  --lang=<lang>      Language: auto, typescript, csharp (default: auto)
  --include-tests    Include test files in dependency analysis (TypeScript only)
  -t                 Short form of --include-tests
  --help, -h         Show this help

Examples:
  create-dependency-graph                         # Auto-detect language
  create-dependency-graph ./my-project            # Specify project path
  create-dependency-graph --lang=csharp           # Force C# mode
...
function generateJSON(files: ParsedFile[], modules: ModuleMap, stats: Statistics, circularDeps: CircularDependencyResult): object {
  ...
  return {
    metadata: {
      ...
      lastUpdated: today,
      totalFiles: stats.totalSourceFiles,
...
/** Top-level namespace prefixes that map to .NET platform (not NuGet) */
const CS_SYSTEM_PREFIXES = new Set(['System', 'Microsoft', 'Windows']);

/**
 * Microsoft.* namespace prefixes that come from NuGet packages rather than the .NET BCL.
 * Each entry is matched as an exact prefix: "Microsoft.Extensions" matches
 * "Microsoft.Extensions.Hosting", "Microsoft.Extensions.DependencyInjection", etc.
 */
const CS_MICROSOFT_NUGET_PREFIXES = [
  'Microsoft.Extensions',
  'Microsoft.Win32.TaskScheduler',
];

function isCsSystemNamespace(ns: string): boolean {
  if (!CS_SYSTEM_PREFIXES.has(ns.split('.')[0])) return false;
  // Microsoft.Extensions.* and Microsoft.Win32.TaskScheduler are NuGet, not BCL
  return !CS_MICROSOFT_NUGET_PREFIXES.some(p => ns === p || ns.startsWith(p + '.'));
}

function isCsInternalNamespace(ns: string, projectNames: string[]): boolean {
  return projectNames.some(p => ns === p || ns.startsWith(p + '.'));
}

/**
 * Infer C# project namespace roots by scanning `src/` for .csproj files.
 * "WindowsMcp.csproj" and "WindowsMcp.Abstractions.csproj" both yield root "WindowsMcp".
 * Falls back to ['Unknown'] if no .csproj files are found.
 */
function inferCsProjectNames(srcDir: string): string[] {
      ...
      try {
        for (const f of readdirSync(join(srcDir, entry.name))) {
          if (f.endsWith('.csproj')) {
            // First component of the csproj name is the namespace root
            // e.g. "WindowsMcp.Abstractions.csproj" → "WindowsMcp"
            roots.add(f.replace('.csproj', '').split('.')[0]);
          }
        }
      } catch { /* skip unreadable subdirs */ }
    }
  } catch { /* srcDir not readable */ }
  return roots.size > 0 ? [...roots] : ['Unknown'];
}

/**
 * Detect whether the project at rootDir uses TypeScript or C# as its primary language.
 * Checks for .sln (C#), src/index.ts (TypeScript), or .csproj files in src/ or its subdirs.
 */
function detectProjectLanguage(rootDir: string): 'typescript' | 'csharp' {
  try {
    if (readdirSync(rootDir).some(f => f.endsWith('.sln'))) return 'csharp';
    if (existsSync(join(rootDir, 'src', 'index.ts'))) return 'typescript';

    const srcDir = join(rootDir, 'src');
    if (existsSync(srcDir)) {
      for (const sub of readdirSync(srcDir)) {
        if (sub.endsWith('.csproj')) return 'csharp';
        const subPath = join(srcDir, sub);
        if (statSync(subPath).isDirectory() &&
            readdirSync(subPath).some(f => f.endsWith('.csproj'))) {
          return 'csharp';
        }
      }
    }
...
/**
 * Recursively collect all .cs files under dir, skipping bin/, obj/, .git/, .vs/
 */
function getAllCsFiles(dir: string, files: string[] = []): string[] {
...
/**
 * Extract a human-readable description from C# XML doc comments or line comments.
 */
function extractCsDescription(content: string): string | null {
  // XML doc <summary> block: /// <summary>\n/// text\n/// </summary>
  const summaryBlock = content.match(/\/\/\/\s*<summary>\s*\n\s*\/\/\/\s*(.+?)\s*\n\s*\/\/\/\s*<\/summary>/);
  if (summaryBlock) return summaryBlock[1].trim();
  // Single-line XML doc without tags
  const xmlLine = content.match(/^\/\/\/\s*(?!<)(.+)$/m);
  if (xmlLine) return xmlLine[1].trim();
  // Regular //
  const regularLine = content.match(/^\/\/\s*(.+)$/m);
  if (regularLine) return regularLine[1].trim();
  return null;
}

/**
 * Infer the C# namespace for a file from its path structure.
 * e.g. src/WindowsMcp/Tools/InputTools.cs → WindowsMcp.Tools
 */
function inferCsNamespace(relativePath: string): string {
  const parts = relativePath.replace(/\\/g, '/').split('/');
  const srcIdx = parts.indexOf('src');
  if (srcIdx < 0) return '';
  // Drop 'src' prefix and the filename; join remaining dirs with '.'
  return parts.slice(srcIdx + 1, -1).join('.');
}

/**
 * Parse a C# source file into a ParsedFile structure compatible with all
 * existing output generators (JSON, YAML, Markdown, Mermaid).
 *
 * Mapping:
 *   using → internalDependencies (internal) / externalDependencies (NuGet) / nodeDependencies (System.*)
 *   interface / class / record / struct → exports.interfaces / exports.classes
 *   enum → exports.enums
 *   [McpServerTool] methods → exports.functions
 */
function parseCsFile(filePath: string, projectNames: string[]): ParsedFile {
  const content = readFileSync(filePath, 'utf-8');
  const relativePath = relative(ROOT_DIR, filePath).replace(/\\/g, '/');

  const result: ParsedFile = {
    path: relativePath,
    name: basename(filePath, '.cs'),
    externalDependencies: [],
    nodeDependencies: [],
    internalDependencies: [],
    exports: {
    ...
    description: extractCsDescription(content),
  };

  // --- Parse using directives ---
  const systemNs = new Set<string>();
  const internalNs = new Set<string>();
  const externalPkgMap = new Map<string, string[]>(); // pkg → [full ns, ...]

  const usingRx = /^using\s+([\w.]+);/gm;
  let m: RegExpExecArray | null;
  while ((m = usingRx.exec(content)) !== null) {
    const ns = m[1];
    if (isCsInternalNamespace(ns, projectNames)) {
      internalNs.add(ns);
    } else if (isCsSystemNamespace(ns)) {
      systemNs.add(ns.split('.')[0]);
    } else {
      const pkg = ns.split('.')[0];
      const list = externalPkgMap.get(pkg) ?? [];
      if (!list.includes(ns)) list.push(ns);
      externalPkgMap.set(pkg, list);
    }
  }

  for (const ns of internalNs) {
    result.internalDependencies.push({ file: ns, imports: [ns] });
  }
  for (const ns of systemNs) {
    result.nodeDependencies.push({ module: ns, imports: [ns] });
  }
  for (const [pkg, nsList] of externalPkgMap) {
    result.externalDependencies.push({ package: pkg, imports: nsList });
  }

  // --- Parse type declarations ---
  // Collect every match of `rx` into `bucket`, and also append to exports.named.
  function collect(rx: RegExp, bucket: string[]): void {
    let mm: RegExpExecArray | null;
    while ((mm = rx.exec(content)) !== null) {
      bucket.push(mm[1]);
      result.exports.named.push(mm[1]);
    }
  }

  collect(/^\s*(?:public\s+)?interface\s+(\w+)/gm, result.exports.interfaces);
  // Classes (covers sealed, abstract, static, partial, internal combinations)
  collect(/^\s*(?:public\s+)?(?:(?:sealed|abstract|static|partial|internal)\s+)*class\s+(\w+)/gm, result.exports.classes);
  // Records (record class, record struct) — categorized as classes
  collect(/^\s*(?:public\s+)?(?:sealed\s+)?record\s+(?:class\s+|struct\s+)?(\w+)/gm, result.exports.classes);
  collect(/^\s*(?:public\s+)?enum\s+(\w+)/gm, result.exports.enums);
  // [McpServerTool] methods — attribute on one line, method signature on the next
  collect(/\[McpServerTool[^\]]*\][^\n]*\n\s*(?:public\s+)?(?:async\s+)?(?:Task(?:<[^>]+>)?|string|void|bool|int)\s+(\w+)\s*\(/gm, result.exports.functions);

  // Deduplicate
  result.exports.named      = [...new Set(result.exports.named)];
  result.exports.interfaces = [...new Set(result.exports.interfaces)];
  result.exports.classes    = [...new Set(result.exports.classes)];
  result.exports.enums      = [...new Set(result.exports.enums)];
  result.exports.functions  = [...new Set(result.exports.functions)];

  return result;
}

/**
 * Categorize C# files into modules by path structure:
 *   Models/     → models
 ...
 *   (root)      → entry
 */
function categorizeCsFiles(files: ParsedFile[]): ModuleMap {
  const modules: ModuleMap = {};
  ...
  for (const file of files) {
    ...
    if (parts.includes('Models')) {
      mod = 'models';
    } else if (parts.some(p => p.includes('Abstractions'))) {
      mod = 'abstractions';
...
/**
 * Build a dependency matrix for C# projects.
 * Since C# imports are namespace-based (not file-path-based), we resolve
 * which files belong to each imported namespace and aggregate importsFrom /
 * exportsTo counts accordingly.
 */
function buildCsDependencyMatrix(files: ParsedFile[]): DependencyMatrix {
  const matrix: DependencyMatrix = {};

  for (const file of files) {
    const importsFrom = file.internalDependencies.map(d => d.file);
    const myNs = inferCsNamespace(file.path);
    const exportsTo = new Set<string>();

    // Which other files import this file's namespace (or a parent of it)?
    for (const other of files) {
      if (other.path === file.path) continue;
      for (const dep of other.internalDependencies) {
        // Match: exact namespace OR a parent namespace (e.g. WindowsMcp covers WindowsMcp.Abstractions)
        if (dep.file === myNs || (myNs !== '' && myNs.startsWith(dep.file + '.'))) {
          exportsTo.add(other.path);
          break;
        }
      }
    ...
    matrix[file.path] = {
      importsFrom: [...new Set(importsFrom)],
      exportsTo:   [...exportsTo],
    };
  }
...
async function main(): Promise<void> {
  ...
  // Detect project language (TypeScript or C#)
  const lang = cliOptions.lang === 'auto'
    ? detectProjectLanguage(ROOT_DIR)
    : cliOptions.lang;
  console.log(`Language: ${lang}`);
  if (lang === 'typescript' && cliOptions.includeTests) {
  ...
  // ── Language-specific scanning and categorization ──
  let parsedFiles: ParsedFile[];
  let modules: ModuleMap;
  let circularDeps: CircularDependencyResult;
  let matrix: DependencyMatrix;

  if (lang === 'csharp') {
    // C# code path — namespace-based imports, no relative-path resolution
    const srcDir = join(ROOT_DIR, 'src');
    const projectNames = inferCsProjectNames(srcDir); // auto-discovered from .csproj files
    console.log(`Scanning codebase for dependencies... (namespace roots: ${projectNames.join(', ')})`);
    const csFiles = getAllCsFiles(srcDir);
    console.log(`Found ${csFiles.length} C# source files`);
    if (csFiles.length === 0) {
      console.error('No C# files found in src/');
      process.exit(1);
    }
    parsedFiles = csFiles.map(f => parseCsFile(f, projectNames));
    console.log('Parsed all C# files');
    modules = categorizeCsFiles(parsedFiles);
    console.log(`Categorized into ${Object.keys(modules).length} modules`);
    // Circular deps are enforced by the C# compiler; skip runtime detection
    circularDeps = { all: [], runtime: [], typeOnly: [] };
    matrix = buildCsDependencyMatrix(parsedFiles);
    console.log('Built C# dependency matrix');
  } else {
    // TypeScript code path (original)
    ...
    parsedFiles = tsFiles.map(parseFile);
    console.log('Parsed all files');
    modules = categorizeFiles(parsedFiles);
    console.log(`Categorized into ${Object.keys(modules).length} modules`);
    circularDeps = detectCircularDependencies(parsedFiles);
    ...
    matrix = buildDependencyMatrix(parsedFiles);
    console.log('Built dependency matrix');
  }

  // Detect unused files and exports — TypeScript only.
  // C# uses namespace-based imports; file-path matching in detectUnused produces
  // false positives (all 63 files appear unused because no path resolves to a namespace string).
  const unusedAnalysis: UnusedAnalysis = (lang === 'csharp')
    ? { unusedFiles: [], unusedExports: [] }
    : detectUnused(parsedFiles);
  ...
  // Test coverage analysis (TypeScript only, when --include-tests is specified)
  ...
  console.log(`  - ${stats.totalSourceFiles} source files analyzed (${lang})`);
