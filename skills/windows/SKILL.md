...
**Files (7)** — read, write, search, and forensically inspect files: `file_read`, `file_write`, `file_manage`, `file_search`, `file_info`, `file_hash`, `file_streams`. Plain absolute paths only — a relative one, or a `\\?\` / `\\.\` device form, is refused naming the parameter. `file_read` pages a large file with `offset_lines`/`limit_lines` instead of raising `max_bytes`; `file_write` takes `append`; `file_manage` refuses an existing copy/move destination without `overwrite: true` — which **replaces** it (the old destination is set aside and only removed once the copy or move succeeds; a failure or a cancel puts it back) — and a non-empty directory delete without `recursive: true`. A volume root, a destination inside its own source, and descending into a junction or symlink are refused outright; deleting a junction removes the link only, and deleting what is not there says so instead of claiming a delete. `list` returns `{Entries:[{Path, Name, IsDirectory, Size, Modified, Hidden, IsLink}], Truncated, MaxEntries}` (`pattern` glob — no path separators, `recursive`, `include_hidden`, `max_entries` default 1000 / max 100000): check `Truncated` before you conclude a directory is fully listed, and `IsLink` marks an entry the walk did not descend into
...
- **Read the error text — it is the server's own answer.** A missing file, a registry key that is not there, a window title that matches nothing (the message lists the open windows), an element id that has expired, an access-denied, a path too long, a refused parameter combination and a timeout all come back **verbatim**, with the path or name in them, capped at 2 000 characters. Act on that sentence instead of retrying blindly. Only an unexpected fault (a null reference, a COM or Win32 failure) is still flattened to `"An error occurred invoking '<tool>'."` — treat *that* one as "something broke", not as "not found".
