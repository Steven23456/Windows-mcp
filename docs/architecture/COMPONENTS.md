...
C-1: `FileRead`, `FileWrite`, `FileManage` (`src` **and** `dst`) and `FileSearch` (`root`) refuse
a path that is not fully qualified (`Path.IsPathFullyQualified`; UNC passes) with one
`ArgumentException` naming the parameter, raised in the tool **before** the service is called —
a relative path would resolve against the server's working directory, which the caller cannot see.
...
| `FileManage` | `copy` / `move` / `delete` (`confirm:true`) / `list`. C-1: `copy`/`move` refuse an existing `dst` unless `overwrite:true` (a directory `src` is copied as a tree; a cross-volume move is a copy then a delete), `delete` refuses a non-empty directory unless `recursive:true`, and `list` returns `FileEntry[]` — `pattern` is a case-insensitive name glob, `recursive` descends, hidden and system entries are skipped unless `include_hidden:true` |
...
- `KillAsync(pid, KillOptions)` runs the start-time guard first when one is given (a mismatch aborts and kills nothing, unchanged), then — only when `Graceful` — posts `WM_CLOSE` through the internal `IProcessWindowNative` seam to every **visible** top-level window the pid owns (`EnumWindows` + `GetWindowThreadProcessId` + `IsWindowVisible` + `PostMessage` in `Win32ProcessWindowNative`; a hidden helper window must not make a windowless process look like one that was asked to close), waits for the exit bounded by `GraceMs`, and calls `Kill()` only if the process is still there. Nothing to post to = forced at once with `ExitedGracefully:false, Forced:true, WaitedMs:0`. The seam and the sample delay are both injected through an `internal` constructor, so the kill path and the sampler are testable with no desktop and no real 250 ms

### Pure helpers (`ScaleMath`, `RegionMath`, `CursorMath`, `CursorOverlay`, `Annotator`, `FlashGlow`, `UiText`, `WindowFilter`, `VirtualDesktopRegistry`, `FuzzyMatch`, `WindowMatcher`, `ForegroundLadder`, `AppCatalog`, `LaunchWait`, `WindowGeometry`, `ArgvJson`, `TypePlanner`, `DragPath`, `BatchTargets`, `RegistryGuard`, `LineWindow`, `CpuSample`)
