...
|-----------|-------------|
| `IInputService` | `ClickAsync`, `DragAsync` (and the B-2 overload `(…, durationMs, steps)`), `HoverAsync`, `TypeAsync` (and the B-1 overload `(text, TypeOptions)` → `TypeResult` with `Method`/`ClipboardRestored`), `PressKeyAsync`, `PressShortcutAsync`, `ScrollAsync` (and the B-3 overload `(…, shiftWheel)`), `GetCursorPositionAsync` → `CursorPosition` |
...
### Pure helpers (`ScaleMath`, `RegionMath`, `CursorMath`, `CursorOverlay`, `Annotator`, `FlashGlow`, `UiText`, `WindowFilter`, `VirtualDesktopRegistry`, `FuzzyMatch`, `WindowMatcher`, `ForegroundLadder`, `AppCatalog`, `LaunchWait`, `WindowGeometry`, `ArgvJson`, `TypePlanner`, `DragPath`)
