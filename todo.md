...
against the upstream Python server (46 prioritized items with implementation sketches) is
...
- **UI-automation / input — needs interactive foreground desktop (21):** `click` · `type` · `key` ·
  `shortcut` · `hover` · `drag` · `scroll` · `focus` · `launch` · `window` · `switch_to_window` ·
  `get_state` · `get_element` · `find_element` · `interact_element` · `assert_element` · `wait_for` ·
  `get_text` · `get_table` · `ocr` · `file_dialog`
