...
class Tree:
    ...
    def get_appwise_nodes(
        ...
        apps = {}
        taskbar = visible_apps.pop("Taskbar", None)
        if taskbar is not None:
            apps["Taskbar"] = taskbar
        program_manager = visible_apps.pop("Program Manager", None)
        if program_manager is not None:
            apps["Program Manager"] = program_manager
        if visible_apps:
    ...
    def annotated_screenshot(
        ...
        # Draw annotations sequentially (PIL ImageDraw is not thread-safe)
